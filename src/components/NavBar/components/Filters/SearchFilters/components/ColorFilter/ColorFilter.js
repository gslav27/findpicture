...
class ImgColor extends Component {
  ...
  selectionRenderer = (native=false) => {
    ...
    let headerLabel = (value) => <span className={this.props.classes.selectedValue}>{value}</span>
    if (!native) {
      switch (values.length) {
        ...
        case 1:
          if (values[0] === null) {
            return (<span>color: {headerLabel('all')}</span>)
          }
          return (<span>color: {headerLabel(values[0])}</span>);
        default:
          return (<span>color: {headerLabel(values.join(', '))}</span>);
      }
    } else {
      switch (values.length) {
        case 0:
